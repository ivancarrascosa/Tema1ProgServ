...
from db.models.profesor import Profesor  # CAMBIADO: importar del modelo correcto
from db.schemas.profesores import profesor_schema, profesores_schema
...
async def add_user(profesor: Profesor):
...
def search_profesor(name: str, surname: str):
    # La búsqueda me devuelve un objeto del tipo de la base de datos.
    # Necesitamos convertirlo a un objeto User. 
    try:
        # Si algo va mal en la búsqueda dentro de la base de datos se lanzará una excepción,
        # así que la controlamos
        profesor = profesor_schema(db_client.test.profesores.find_one({"nombre":name, "apellidos":surname}))
        return Profesor(**profesor)
    except:
        return {"error": "Profesor not found"}
